...
atoms_Si = POSCAR.get_bulk_structure('Si').atoms
atoms_Si.set_cell([[lattice_constant, 0, 0], [0, lattice_constant, 0], [0, 0, lattice_constant]], scale_atoms=True)
atoms_Si.calc = macemp
atoms_optimiser = FIRE(atoms_Si)
atoms_optimiser.run(fmax=0.001)
...
for i in [1.1, 1.09, 1.08, 1.07, 1.06, 1.05, 1.04, 1.03, 1.02, 1.01, 1.0, 0.99, 0.98, 0.97, 0.96, 0.95, 0.94, 0.93, 0.92, 0.91, 0.90]:
    ...
    atoms_Si_strained.set_cell([[lattice_constant * i, 0, 0], [0, lattice_constant * i, 0], [0, 0, lattice_constant * i]], scale_atoms=True)
    ...
    write(f"structures/strain/Si_a_{i}.vasp", atoms_Si_strained, 'vasp')
    ...
    results.append([i, lattice_constant * i, E_Si_strained, E_Si_strained_per_atom, E_Si_strain, E_Si_strain_per_atom])
