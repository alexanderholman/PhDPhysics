...
poscar.write(f"expanded-12-12-1.vasp")

# Read the generated structure from a VASP POSCAR file.
poscar.load_into_ace()

# Take snapshot of the structure, pre-relaxation.
poscar.image(f"pre-relaxation-expanded-12-12-1.png")

# Relax the structure.
poscar.relax(
    write=True,
    filename=f"relaxed-expanded-12-12-1.vasp"
)

# Take snapshot of the structure, post-relaxation.
poscar.image(f"post-relaxation-expanded-12-12-1.png")

# Write the energy of the relaxed structure.
poscar.write_energy(filename=f"energy-expanded-12-12-1.txt")
